...
const showPlayerPosition = () => {
  playerPositionElement.innerText = `Player position\
      x: ${stateOfGame.player.position.x}\
      y: ${stateOfGame.player.position.y}`;
};
showPlayerPosition();

const monstersListElement = document.getElementById("monstersList");

const changeInfoAboutMonsters = (infoAboutMonsters, monsterName) => {
  if (monsterName !== null) {
    if (monsterName in infoAboutMonsters) {
      infoAboutMonsters[monsterName]++;
    } else {
      infoAboutMonsters[monsterName] = 1;
    }
  }
};

const showListOfMonsters = () => {
  const infoAboutCountOfMonsters = {};

  for (let i = 0; i < gameField.length; i++) {
    for (let j = 0; j < gameField[i].length; j++) {
      changeInfoAboutMonsters(infoAboutCountOfMonsters, gameField[i][j]);
    }
  }

  monstersListElement.innerHTML = "";
  for (const monsterName in infoAboutCountOfMonsters) {
    const listElements = document.createElement("li");
    listElements.innerText = `${monsterName} - ${infoAboutCountOfMonsters[monsterName]}`;
    monstersListElement.appendChild(listElements);
  }
};
